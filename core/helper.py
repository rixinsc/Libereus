...
class Logger(object):
    ...
    def __init__(self):
        ...
        self.logger = logging.getLogger('Libereus')
        self.logger.setLevel(logging.INFO)
        #setting stream handler
        streamhandler = logging.StreamHandler()
        streamhandler.setFormatter(formatter)
        #setting filehandler
        filehandler = logging.FileHandler('Libereus.log')
...
async def decheck(ctx, err) -> bool:
        ...
        if isinstance(err, commands.errors.BotMissingPermissions):
                logger.error(content=notberr, reason="bot missing permission")
                await ctx.send("I need `{}` permission to do this.".format(err.missing_perms))
                return True
        if isinstance(err, commands.CommandOnCooldown):
                logger.error(content=tberr, reason=f'command is currently on cooldown type {err.cooldown.type}')
        ...
        if isinstance(err, (commands.errors.MissingRequiredArgument, commands.errors.BadArgument, commands.errors.TooManyArguments)): #using command improperly
                logger.error(content=notberr, reason="command argument error")
                #send help
        ...
        if isinstance(err, discord.Forbidden):
                logger.error(content=tberr, reason='received 403 Forbidden')
